...
import { DatabaseHandler, FileHandler } from '@src/handlers'
...
export const connect = () =>
	config.get<string>('database.path').pipe(
		Effect.flatMap(FileUtils.checkFile),
		Effect.matchEffect({
			onSuccess: dir => Effect.succeed(dir),
			onFailure: error => {
				if (error instanceof FileHandler.AccessError && error.code === 'ENOENT')
					return FileUtils.makeDir(error.file) // create directory if inexistant
				return Effect.fail(error)
			},
		}),
		Effect.flatMap(directory =>
			FileUtils.checkFile(path.join(directory, 'echo.sqlite')).pipe(
